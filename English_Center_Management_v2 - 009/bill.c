...
int printAllBillCallback(const Bill* bl) {
    printBill(bl);
    puts("");
    return CONTINUE;
}
...
Bill* copyBill(Bill* destination, const Bill* source) {
    ...
    destination->soTien = source->soTien;
    destination->transactionTime = source->transactionTime;
...
typedef struct compareIDwithBillPacket {
    int id;
    int* result;
} compareIDwithBillPacket;

int compareIDwithBillCallback(const Bill* bl, compareIDwithBillPacket* packet) {
...
int isBillExist(int billID, const LinkedList* billList) {
    int listState = isListEmpty(billList);
    if (listState || listState == -1) return 0;

    int result = 0;
    compareIDwithBillPacket packet = {.id = billID, .result = &result};
    iterList(billList, (listCallback) compareIDwithBillCallback, (void*) &packet);
...
int isBillSubjectExist(const Bill* bl, const BillSubjectValidationContext ctx) {
    ...
    Subject sender = bl->sender;
    switch(sender.sbj_type) {
        case TEACHER: {
            int result = ctx.teacherContext.checker(sender.id, ctx.teacherContext.list);
            if(!result) {
                printf("Khong ton nguoi gui la tai giao vien voi id: %d\n", sender.id);
                return 0;
            }
            break;
        }
        case STUDENT: {
            int result = ctx.studentContext.checker(sender.id, ctx.studentContext.list);
            if(!result) {
                printf("Khong ton tai nguoi gui la hoc vien voi id: %d\n", sender.id);
                return 0;
            }
            break;
        }
    ...
    Subject receiver = bl->receiver;
    switch(receiver.sbj_type) {
        case TEACHER: {
            int result = ctx.teacherContext.checker(receiver.id, ctx.teacherContext.list);
            if(!result) {
                printf("Khong ton tai nguoi nhan la giao vien voi id: %d\n", receiver.id);
                return 0;
            }
            break;
        }
        case STUDENT: {
            int result = ctx.studentContext.checker(receiver.id, ctx.studentContext.list);
            if(!result) {
                printf("Khong ton tai nguoi nhan la hoc vien voi id: %d\n", receiver.id);
                return 0;
            }
            break;
        }

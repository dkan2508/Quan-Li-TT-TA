...
HocVien* copyStudent(HocVien* destination, const HocVien* source) {
    if(!destination | !source) return NULL;

    destination->id = source->id;
    strcpy(destination->ten, source->ten);
    destination->tuoi = source->tuoi;
...
int freeStudentCallback(HocVien* hv, void* placeholder) {
    if(!hv) return CONTINUE;
    free(hv);
    hv = NULL;
    return CONTINUE;
}

int printAllStudentCallback(const HocVien* hv) {
    printStudent(hv);
    puts("");
    return CONTINUE;
}
...
typedef struct compareIDwithStudentPacket {
    int id;
    int* result;
} compareIDwithStudentPacket;

int compareIDwithStudentCallback(const HocVien* bl, compareIDwithStudentPacket* packet) {
...
int isStudentExist(int studentID, const LinkedList* studentList) {
    int listState = isListEmpty(studentList);
    if (listState || listState == -1) return 0;

    int result = 0;
    compareIDwithStudentPacket packet = {.id = studentID, .result = &result};
    iterList(studentList, (listCallback) compareIDwithStudentCallback, (void*) &packet);

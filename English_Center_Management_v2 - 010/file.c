...
int resetManagerFile() {
    mkdir(MANAGING_DIR); // Tao thu muc neu chua co
...
int capNhatFileQuanLi(SUBJECT_FILE_TYPE option, char newPath[]) {
    ...
    noiDungFile[option][MAX_FILE_PATH_LENGTH - 1] = '\0';
...
int setAutoModeLine(int line, int option) {
    ...
    // Update line
    free(lines[line]); // Free the old string to make place for the new string.
    lines[line] = malloc(2);
...
int linkedListSerializationCallback(const void* data, linkedListSerializationPacket* packet) {
...
cJSON* parseJSONArrToObj(char path[], int* size) { // Chuyen tu dang json string luu trong file ra json arr
    FILE* f = fopen(path, "r");
    if (!f) {
        return NULL;
    }

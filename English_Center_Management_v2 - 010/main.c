...
int main() {
        ...
        switch (luaChon) {
                ...
                do {
                    ...
                    switch (luaChonCon) {
                        ...
                        case 2: {
                            ...
                            if(isStudentExist(getStudentID(newStudent), studentList)) {
                                printf("Da ton tai hoc vien voi id: %d\n", getStudentID(newStudent));
                                free(newStudent);
                                break;
                            }
                        ...
                        case 4: {
                            ...
                            if(filteredList) {
                                HocVien* temp = createStudent();
                                copyStudent(temp, filteredList[0]);
                                puts("\nChon thong tin can dieu chinh:");
                                int modResult = enterStudentInfo(temp, 1);
                                if(modResult) {
                                    if(isStudentExist(getStudentID(temp), studentList) && getStudentID(temp) != getStudentID(filteredList[0])) {
                                        printf("Da ton tai hoc vien voi id: %d\n", getStudentID(temp));
                                        free(temp);
                                        free(filteredList);
                                        break;
                                    ...
                                    copyStudent(filteredList[0], temp);
                ...
                do {
                    ...
                    switch (luaChonCon) {
                        ...
                        case 2: {
                            ...
                            if(isTeacherExist(getTeacherID(newTeacher), teacherList)) {
                                printf("Da ton tai giang vien voi id: %d\n", getTeacherID(newTeacher));
                                free(newTeacher);
                                break;
                            }
                        ...
                        case 4: {
                            ...
                            if(filteredList) {
                                GiangVien* temp = createTeacher();
                                if(!temp) {
                                    free(filteredList);
                                    break;
                                }
                                copyTeacher(temp, filteredList[0]);

                                puts("\nChon thong tin can dieu chinh:");
                                int modResult = enterTeacherInfo(temp, 1);
                                if(modResult) {
                                    if(isTeacherExist(getTeacherID(temp), teacherList) && getTeacherID(temp) != getTeacherID(filteredList[0])) {
                                        printf("Da ton tai giao vien voi id: %d\n", getTeacherID(temp));
                                        free(temp);
                                        free(filteredList);
                                        break;
                                    }
                                    puts("Thay doi thong tin thanh cong");
                                    copyTeacher(filteredList[0], temp);
                                } else puts("Thay doi thong tin that bai");
                                free(temp);
                                free(filteredList);
                                break;
                ...
                do {
                    ...
                    switch (luaChonCon) {
                        ...
                        case 2: {
                            ...
                            if(isCourseExist(getCourseID(newCourse), courseList)) {
                                printf("Da ton tai khoa hoc voi id: %d\n", getCourseID(newCourse));
                                free(newCourse);
                                break;
                            }
                        ...
                        case 4: {
                            ...
                            if(filteredList) {
                                KhoaHoc* temp = createCourse();
                                if(!temp) {
                                    free(filteredList);
                                    break;
                                }
                                copyCourse(temp, filteredList[0]);
                                puts("\nChon thong tin can dieu chinh:");
                                int modResult = enterCourseInfo(temp, 1);
                                if(modResult) {
                                    if(isCourseExist(getCourseID(temp), courseList) && getCourseID(temp) != getCourseID(filteredList[0])) {
                                        printf("Da ton tai khoa hoc voi id: %d\n", getCourseID(temp));
                                        free(temp);
                                        free(filteredList);
                                        break;
                                    ...
                                    puts("Thay doi thong tin thanh cong");
                                    copyCourse(filteredList[0], temp);
                                } else puts("Thay doi thong tin that bai");
                                free(temp);
                                free(filteredList);
                                break;
            ...
            case 4: {
                do {
                    ...
                    printf("6. In danh sach hoc sinh\n");
                    printf("7. Quay lai\n");
                    printf("Nhap lua chon: ");
                    saferScanf("%d", &luaChonCon);
                    ...
                    switch (luaChonCon) {
                        ...
                        case 2: {
                            ...
                            if(isClassExist(getClassID(newClass), classList)) {
                                printf("Da ton tai lop hoc voi id: %d\n", getClassID(newClass));
                                free(newClass);
                                break;
                            }
                            ClassSubjectValidationContext ctx = {
                                .courseContext = {.checker = (classSubjectChecker) isCourseExist, .list = courseList},
                                .teacherContext = {.checker = (classSubjectChecker) isTeacherExist, .list = teacherList},
                            };
                            if(isClassSubjectExist(newClass, ctx)) {
                        ...
                        case 4: {
                            ...
                            if(filteredList) {
                                LopHoc* temp = createClass();
                                copyClass(temp, filteredList[0]);
                                ClassSubjectValidationContext ctx = {
                                    .courseContext = {.checker = (classSubjectChecker) isCourseExist, .list = courseList},
                                    .teacherContext = {.checker = (classSubjectChecker) isTeacherExist, .list = teacherList},
                                };
                                ...
                                puts("\nChon thong tin can dieu chinh:");
                                if(!enterClassInfo(temp, 1)) {
                                    puts("Thay doi thong tin that bai");
                                    free(filteredList);
                                    free(temp);
                                    break;
                                }
                                if(isClassExist(getClassID(temp), classList) && getClassID(temp) != getClassID(filteredList[0])) {
                                    printf("Da ton tai lop hoc voi id: %d\n", getClassID(temp));
                                    free(filteredList);
                                    free(temp);
                                    break;
                                }
                                if(isClassSubjectExist(temp, ctx)) {
                                    puts("Thay doi thong tin thanh cong!");
                                    copyClass(filteredList[0], temp);
                                }
                        ...
                        case 6: {
                            ...
                            if(filteredList) {
                                printStudentList(filteredList[0]);
                                free(filteredList);
                                break;
                            }
                            puts("Tim kiem lop hoc that bai!");
                ...
                do {
                    ...
                    switch (luaChonCon) {
                        ...
                        case 2: {
                            ...
                            if(!enterBillInfo(newBill, 0)) {
                                puts("Nhap thong tin khong hoan thanh.");
                                free(newBill);
                                break;
                            }
                            if(isBillExist(getBillID(newBill), billList)) {
                                printf("Da ton tai hoa don voi id: %d\n", getBillID(newBill));
                                free(newBill);
                                break;
                            }
                            BillSubjectValidationContext ctx = {
                                .studentContext = {.checker = (billSubjectChecker) isStudentExist, .list = studentList},
                                .teacherContext = {.checker = (billSubjectChecker) isTeacherExist, .list = teacherList},
                            };
                            if(isBillSubjectExist(newBill, ctx)) {
                        ...
                        case 4: {
                            ...
                            if(filteredList) {
                                Bill* temp = createBill();
                                copyBill(temp, filteredList[0]);
                                BillSubjectValidationContext ctx = {
                                    .studentContext = {.checker = (billSubjectChecker) isStudentExist, .list = studentList},
                                    .teacherContext = {.checker = (billSubjectChecker) isTeacherExist, .list = teacherList},
                                };
                                puts("\nChon thong tin can dieu chinh:");
                                if(!enterBillInfo(temp, 1)) {
                                    puts("Thay doi thong tin that bai");
                                    free(filteredList);
                                    free(temp);
                                    break;
                                }
                                if(isBillExist(getBillID(temp), billList) && getBillID(temp) != getBillID(filteredList[0])) {
                                    printf("Da ton tai hoa don voi id: %d\n", getBillID(temp));
                                    free(filteredList);
                                    free(temp);
                                    break;
                                }
                                if(isBillSubjectExist(temp, ctx)) {
                                    puts("Thay doi thong tin thanh cong!");
                                    copyBill(filteredList[0], temp);
                                }
                                free(filteredList);
                                free(temp);
                                break;
    ...
    iterList(studentList, (listCallback) freeStudentCallback, NULL);
    destroyList(studentList);

    iterList(teacherList, (listCallback) freeTeacherCallback, NULL);
    destroyList(teacherList);

    iterList(courseList, (listCallback) freeCourseCallback, NULL);
    destroyList(courseList);

    iterList(classList, (listCallback) freeClassCallback, NULL);
    destroyList(classList);

    iterList(billList, (listCallback) freeBillCallback, NULL);

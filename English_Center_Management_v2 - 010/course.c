...
KhoaHoc* copyCourse(KhoaHoc* destination, const KhoaHoc* source) {
    if(!destination || !source) return NULL;

    destination->hocPhi = source->hocPhi;
...
typedef struct compareIDwithCoursePacket {
    int id;
    int* result;
} compareIDwithCoursePacket;

int compareIDwithCourseCallback(const KhoaHoc* bl, compareIDwithCoursePacket* packet) {
...
int isCourseExist(int courseID, const LinkedList* courseList) {
    int listState = isListEmpty(courseList);
    if (listState || listState == -1) return 0;

    int result = 0;
    compareIDwithCoursePacket packet = {.id = courseID, .result = &result};
    iterList(courseList, (listCallback) compareIDwithCourseCallback, (void*) &packet);

    return result;
}

int getCourseID(const KhoaHoc* cr) {
    if(!cr) return 0;
    return cr->id;
}
...
int freeCourseCallback(KhoaHoc* crs, void* placeholder) {
    if(!crs) return CONTINUE;
    free(crs);
    crs = NULL;
    return CONTINUE;
}

int printAllCourseCallback(const KhoaHoc* kh) {
    printCourse(kh);
    puts("");
    return CONTINUE;
}
